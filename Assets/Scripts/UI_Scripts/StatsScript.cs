...
public class StatsScript : MonoBehaviour {
    ...
    public void CreateStatText(MainMenu Caller)
    {
        ...
        YScale = GetComponent<RectTransform>().localScale.y;
    ...
    {
        GameObject Txt = (GameObject)Instantiate(Resources.Load("TxtPrefab"));
        Txt.transform.SetParent(ScrollViewContent);
        Txt.name = name;
    ...
    void SetScrollViewRect()
    {
        ...
        float SVtop = RT.rect.yMax - RT.rect.height * 0.2f;
        float SVbottom = RT.rect.yMin + RT.rect.height * 0.1f;
        float SVleft = RT.rect.xMin + RT.rect.width * 0.08f;
        float SVright = SVleft + RT.rect.width * 0.6f;

        float height = SVtop - SVbottom;
        float width = SVright - SVleft;

        float Left = (SVleft + width / 2) * XScale;
        float Top = (SVtop - height / 2) * YScale;

        ScrollView.sizeDelta = new Vector2(width, height);
        ScrollView.position = new Vector2(Left, Top);
    }
